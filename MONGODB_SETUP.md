...
- **Render: TLS / "tlsv1 alert internal error" (SSL alert 80):** This is an SSL handshake failure between Render and MongoDB Atlas. Try in order:
  1. **Atlas Network Access:** In MongoDB Atlas → Network Access → ensure **0.0.0.0/0** is allowed (so Render’s outbound IPs can connect).
  2. **Connection string:** Use the URI from Atlas “Connect your application” (Driver: Node.js). If the password has special characters, URL-encode them (e.g. `@` → `%40`, `#` → `%23`).
  3. **Node version on Render:** In Render dashboard → Backend service → Environment, set **NODE_VERSION=20** (or use `engines.node` in `apps/backend/package.json`). Node 20 LTS often avoids this TLS issue compared to Node 22.
  4. **App behavior:** The backend still starts if MongoDB fails: it uses file storage and retries MongoDB every 30s. Check logs for "Using MongoDB storage from startup" or "MongoDB connect failed (will retry in 30s)" and later "Switched to MongoDB storage".